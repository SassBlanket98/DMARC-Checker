...
async function checkRecord() {
  ...
  // Reset overview container - UPDATED LINE
  ...
  try {
    ...
    if (recordType === "overview") {
      // Display overview score container - UPDATED CODE
      ...
      // Debug information to help diagnose any remaining issues
      console.log("Overview records:", data.records);
      console.log("Score methodology available:", !!window.ScoreMethodology);
    } else {
...
function updateOverviewDashboard(records) {
  ...
  console.log("Updating overview dashboard with records:", records);
  ...
  try {
    ...
    if (scoreCircle && scoreValue) {
      ...
      console.error("Score circle or value element not found in the DOM");
    }
    ...
    Object.entries(componentScores).forEach(([component, data]) => {
      ...
      if (componentItem) {
        ...
        console.warn(`Component item for ${component} not found in the DOM`);
      }
    });
    ...
    if (!document.getElementById("view-score-details")) {
      ...
      if (scoreLabel) {
        ...
        console.warn("Score label element not found");
      }
    }
  } catch (error) {
    console.error("Error calculating score:", error);
    showToast("Error calculating authentication score", "error");
  }
}
